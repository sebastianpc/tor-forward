...
impl PortsBuilder {
    ...
    pub fn forward(mut self, remote: u16, local: u16) -> PortsBuilder {
        ...
        let rule = ProxyRule::new(ProxyPattern::one_port(remote).unwrap(), ProxyAction::Forward(Encapsulation::Simple, TargetAddr::Inet(socket_addr)));
    ...
    pub fn expose(mut self, port: u16) -> PortsBuilder {
        ...
        let rule = ProxyRule::new(ProxyPattern::one_port(port).unwrap(), ProxyAction::Forward(Encapsulation::Simple, TargetAddr::Inet(socket_addr)));
        self.rules.push(rule);
        self
    }
